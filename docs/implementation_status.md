...
- **変更点（予定）**:
  - `ResearcherAgent` の「キーワード検索」を **Tavily検索からRSS集約検索**へ置換する。
  - 事前に用意した **RSSフィードURLの許可リスト**（設定ファイル or 環境変数）から直近記事を取得し、タイトル/概要/本文の一部でキーワードマッチさせて候補URLを選定する。
  - 候補URLの本文取得は既存のURL取得ロジックを流用する（RSSに掲載されたリンクのみを対象とする）。
- **設計上の安全策**:
