...
2. **FactCheckerの構造化安定化**
   - `Critique` が一部キー欠落でパース失敗するケースがあるため、JSONフォールバックの強化やプロンプト制約の見直し。
3. **記事メタデータの精度向上**
   - URL取得時にタイトル/ソースを抽出して `article_info` をより正確にする（現状はURL時タイトルが不明になりがち）。
4. **RSS検索の精度向上（任意）**
   - ドメイン別本文抽出チューニング、フィードの拡充、クエリ→候補選定の改善。
5. **テスト拡充（任意）**
   - RSSフィード読み込み/ランキングの単体テスト、LangGraphの統合テストを追加。
